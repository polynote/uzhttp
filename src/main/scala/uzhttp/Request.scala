...
object Request {
  ...
  object Method {
    val Methods: List[Method] =
      List(GET, HEAD, POST, PUT, DELETE, TRACE, OPTIONS, CONNECT, PATCH)
    ...
    def parseEither(str: String): Either[BadRequest, Method] = {
      ...
      Methods
        .find(_.name == strU)
        .map(Right(_))
        .getOrElse(Left(BadRequest("Invalid method")))
    }
  ...
  private[uzhttp] final case class ReceivingBody(
      method: Method,
      uri: URI,
      version: Version,
      headers: Headers,
      bodyQueue: Queue[Take[HTTPError, Byte]],
      received: Ref[Long],
      contentLength: Long
  ) extends ContinuingRequest {
    override val body: Option[Stream[HTTPError, Byte]] = Some(
      Stream.fromQueue(bodyQueue).flattenTake
    )
    override val noBufferInput: Boolean = false
    override def addHeader(name: String, value: String): Request =
      copy(headers = headers + (name -> value))
    override def removeHeader(name: String): Request =
    ...
    override def submitBytes(chunk: Chunk[Byte]): UIO[Unit] = bodyQueue.offer(
      Take.chunk(chunk)
    ) *> received.updateAndGet(_ + chunk.length).flatMap {
      case received if received >= contentLength =>
        bodyQueue.offer(Take.end).unit
      case _ => ZIO.unit
    }
  }

  private[uzhttp] object ReceivingBody {
    private[uzhttp] def create(
        method: Method,
        uri: URI,
        version: Version,
        headers: Headers,
      ...
      (Queue.unbounded[Take[HTTPError, Byte]] <*> Ref.make[Long](0L)).map {
        case (body, received) =>
          new ReceivingBody(
            method,
            uri,
  ...
  private final case class ConstBody(
      method: Method,
      uri: URI,
      version: Version,
      headers: Headers,
    ...
    override def body: Option[Stream[Nothing, Byte]] = Some(
      Stream.fromChunk(bodyChunk)
    )
    override def addHeader(name: String, value: String): Request =
      copy(headers = headers + (name -> value))
    override def removeHeader(name: String): Request =
  ...
  private[uzhttp] final case class NoBody(
      method: Method,
      uri: URI,
      version: Version,
      headers: Headers
  ...
  private[uzhttp] object NoBody {
    def fromReqString(str: String): Either[BadRequest, NoBody] =
      str.linesWithSeparators
        ...
        case first :: rest =>
          first.split(' ').toList match {
            case List(methodStr, uri, versionStr) =>
              for {
                uri <-
                  try Right(new URI(uri))
                  catch {
                    case _: Throwable =>
                      Left(BadRequest("Malformed request URI"))
                  }
  ...
  final class WebsocketRequest(
    ...
    override def removeHeader(name: String): Request = new WebsocketRequest(
      method,
      uri,
      version,
      headers = headers.removed(name),
      chunks
    )
    override val body: Option[Stream[HTTPError, Byte]] = None
    override val bytesRemaining: UIO[Long] = ZIO.succeed(Long.MaxValue)
    override def submitBytes(chunk: Chunk[Byte]): UIO[Unit] =
      chunks.offer(Take.chunk(chunk)).unit
    override def channelClosed(): UIO[Unit] = chunks.offer(Take.end).unit
    override val noBufferInput: Boolean = true
    lazy val frames: Stream[Throwable, Frame] =
      Frame.parse(ZStream.fromQueue(chunks).flattenTake)
  }
  ...
  object WebsocketRequest {
    def apply(
        ...
        version: Version,
        headers: Headers
    ): UIO[WebsocketRequest] =
      Queue.unbounded[Take[Nothing, Byte]].map { chunks =>
        new WebsocketRequest(method, uri, version, headers, chunks)
      }

    def unapply(
        req: Request
    ): Option[(Method, URI, Version, Headers, Stream[Throwable, Frame])] =
      req match {
        case req: WebsocketRequest =>
          Some((req.method, req.uri, req.version, req.headers, req.frames))
        case _ => None
      }
  }
