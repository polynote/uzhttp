...
import zio.stream.{Sink, Stream, StreamChunk, Take, ZSink, ZStream}


object Websocket {

  final case class WebsocketResponse private[server](
    ...
    override private[server] def closeAfter = true
  }
  ...
  object Frame {
    ...
    def parse(stream: Stream[Throwable, Chunk[Byte]]): Stream[Throwable, Frame] = stream.aggregate(parseFrame)
  ...
  private def frameSize(payloadLength: Int) =
  ...
  private def writeLength(len: Int, buf: ByteBuffer) =
    if (len < 126) {
      buf.put(len.toByte)
    } else if (len <= 0xFFFF) {
      buf.put(126.toByte)
  ...
  private def frameBytes(op: Byte, payload: Array[Byte], fin: Boolean = true) = {
